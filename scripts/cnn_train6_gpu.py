...
folderPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/data6'
logPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/trainingLog6.csv'
checkpointPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/checkpoints6'
validationSplit = 0.2
...
#
class trainingRead:
	def __init__(self, sixMers, eventMean, eventStd, stutter, lengthsMean, lengthsStd, llScore, modelMeans, readID, analogueConc):
		self.sixMers = sixMers[0:maxLen]
		self.eventMean = eventMean[0:maxLen]
		self.eventStd = eventStd[0:maxLen]
		self.stutter = stutter[0:maxLen]
		self.lengthsMean = lengthsMean[0:maxLen]
		self.lengthsStd = lengthsStd[0:maxLen]
		self.llScore = llScore[0:maxLen]
		self.modelMeans = modelMeans
		self.readID = readID
		self.analogueConc = analogueConc

		if not len(sixMers) == len(eventMean) == len(eventStd) == len(stutter) == len(lengthsMean) == len(lengthsStd) == len(llScore) == len(modelMeans):
			print('PROBLEM')
...
def trainingReadToTensor(t):
	...
	for i, s in enumerate(t.sixMers):
		...
		oneHot.append(t.lengthsMean[i])
		oneHot.append(t.lengthsStd[i])
	...
	for line in f:
		if line[0] == '>':
			...
			if not first and len(sixMers) > maxLen:
				...
				tr = trainingRead(sixMers, eventMean, eventStd, stutter, lengthsMean, lengthsStd, llScore, modelMeans, readID, analogueConc)
				saveRead(tr, readID)
			first = False
			...
			sixMer = splitLine[0]
			if sixMer == prevSixMer:
				events_current.append(float(splitLine[1]))
				lengths_current.append(float(splitLine[2]))
			elif prevSixMer != '': #not the first one
				sixMers.append(prevSixMer)
				eventMean.append(np.mean(events_current))
				eventStd.append(np.std(events_current))
				stutter.append(len(events_current))
				lengthsMean.append(np.mean(lengths_current))
				lengthsStd.append(np.std(lengths_current))
				llScore.append(logLikelihood)
				modelMeans.append(mm)
			...
			if len(splitLine) == 5:
				logLikelihood = splitLine[4]
			else:
				logLikelihood = '-'
			mm = float(splitLine[3])
...
#CNN architecture
model = Sequential()
model.add(Conv1D(32,(16),padding='same',input_shape=(None,10)))
