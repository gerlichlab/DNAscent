...
folderPathBrdU = '/home/mb915/rds/rds-mb915-notbackedup/data/2021_05_21_FT_ONT_Plasmodium_BrdU_EdU/BrdU_trainingData/splitData_CNNbootstrap_raw'

trainingFilesFolderPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2021_05_21_FT_ONT_Plasmodium_BrdU_EdU/BrdUinEdU_trainingData_raw/gap40'
...
def fetchMatchingRead(fname, baseChromosome, baseStart, baseEnd, baseStrand,usedIDs):
	...
	for line in g:
		...
		if line[0] == '>':
			...
			if chromosome != 'chrM' and baseChromosome == chromosome and baseStrand == strand and min(mappingStart,mappingEnd) <= baseStart and max(mappingStart,mappingEnd) >= baseEnd and readID not in usedIDs and readID in analoguePositiveIDs_EdU:
				switch = True
			else:
				switch = False
			...
			if pos != prevPos and prevPos != -1:
				...
				refPos2feature[prevPos] = (sixMer,pos_eventMeans[0:maxRaw],modelMean,modelStd,BrdUcall)
...
for matchChr in set2Chromosomes[int(sys.argv[1])]:
	...
	for strand in ['fwd','rev']:
		for m in marks:
			...
			bc08dnascent = folderPathEdU+'/EdU_'+matchChr+'_'+strand+'_'+str(m)+'.trainingData'
			...
			for line in f:
				...
				if line[0] == '>':
						...
						if switch:
							...
							if len(matchingReadDic) > 0:
								...
								for s in range(numReadSlices):
									...
									tr = trainingRead(augmented_sixMers[maxLen*s:maxLen*(s+1)], augmented_eventMean[maxLen*s:maxLen*(s+1)],  augmented_modelMeans[maxLen*s:maxLen*(s+1)], augmented_modelStd[maxLen*s:maxLen*(s+1)], augmented_logLikelihood[maxLen*s:maxLen*(s+1)], prevReadID+'.'+matchID+'_slice'+str(s), -3)
					...
					if chromosome != 'chrM' and readID in analoguePositiveIDs_BrdU:
						switch = True
					else:
						switch = False
