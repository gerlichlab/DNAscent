...
trainingFilesFolderPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2021_05_21_FT_ONT_Plasmodium_BrdU_EdU/EdUinBrdU_trainingData_LSTMevents/gap40'
	...
	for line in g:
		...
		if line[0] == '>':
			...
			if pos != prevPos and prevPos != -1:
				...
				refPos2feature[prevPos] = (sixMer,pos_eventMeans,pos_lengths,modelMean,modelStd,EdUcall)
						...
						if switch:
							...
							if len(matchingReadDic) > 0:
								...
								for s in range(numReadSlices):
									...
									tr = trainingRead(augmented_sixMers[maxLen*s:maxLen*(s+1)], augmented_eventMean[maxLen*s:maxLen*(s+1)],  augmented_eventLength[maxLen*s:maxLen*(s+1)], augmented_modelMeans[maxLen*s:maxLen*(s+1)], augmented_modelStd[maxLen*s:maxLen*(s+1)], augmented_logLikelihood[maxLen*s:maxLen*(s+1)], prevReadID+'.'+matchID+'_slice'+str(s), -3)
