...
f_checkpoint = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/checkpointsCTC_7_pt3/weights.11.h5'
...
def buildModel():
	...
	X = Conv1D(256, 4, strides = 1, padding='same', name = 'conv2', kernel_initializer = glorot_uniform(seed=0))(X)
...
prediction_model = Model(
    model.get_layer(name="the_input").input, model.get_layer(name="time_distributed").output
)


#for i, ID in enumerate(val_readIDs):
f = open(val_readIDs[1], "rb")
trainingRead = pickle.load(f)
f.close()
tensor = trainingReadToTensor(trainingRead)
tensor = tensor.reshape(tensor.shape[0],1,2)
print(prediction_model.predict(tensor)[0:100])
