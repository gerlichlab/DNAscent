...
logPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/trainingLog_alignment11.csv'
trainingReadLogPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/trainingReadsUsed_alignment11.txt'
valReadLogPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/valReadsUsed_alignment11.txt'
checkpointPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/alignment11'
validationSplit = 0.2

f_checkpoint = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/alignment11/weights.01-0.11.h5'
...
#
poreModel = {}
f_pm = '/home/mb915/rds/hpc-work/development/DNAscent_dev/pore_models/template_median68pA.6mer.model'
f_pm_handle = open(f_pm,'r')
for line in f_pm_handle:
	if line[0] == 'k':
		continue
...
def trainingReadToTensor(t):
	...
	for i, s in enumerate(t.sixMers[0:500]):
		...
		sequenceTensor.append(poreModel[s])
		
	sequenceTensor = np.array(sequenceTensor)			
...
def buildModel(input_shape = (64, 64, 3), classes = 6):
	...
	input_baseSequence = Input(name='bases_input',shape=(None,1))

	#events stack
	E = Masking(mask_value=0.0)(input_events)
	E = Bidirectional(LSTM(16, return_sequences=True))(E)
	E = Bidirectional(LSTM(16, return_sequences=True))(E)
	E = Bidirectional(LSTM(16, return_sequences=True))(E)
	
	#base sequence stack
	S = Masking(mask_value=0.0)(input_baseSequence)
...
class DataGenerator(Sequence):
	...
	def __data_generation(self, list_IDs_temp):
		...
		for i, ID in enumerate(list_IDs_temp):
			...
			y.append(label)
			w.append(weights)
			if eventTensor.shape[0] > maxEvLen:
				maxEvLen = eventTensor.shape[0]

		E = pad_sequences(E, dtype='float32', value=0,maxlen=maxEvLen,padding='post')
		S = pad_sequences(S, dtype='float32', value=0,maxlen=maxEvLen,padding='post')
		y = pad_sequences(y, dtype='float32', value=0,maxlen=maxEvLen,padding='post')
		w = pad_sequences(w, dtype='float32', value=0,maxlen=maxEvLen,padding='post')

		E = np.array(E).reshape(self.batch_size,maxEvLen,2)
		S = np.array(S).reshape(self.batch_size,maxEvLen,1)
		y = np.array(y).reshape(self.batch_size,maxEvLen,4)
		w = np.array(w).reshape(self.batch_size,maxEvLen)
...
f = open(val_readIDs[0], "rb")
trainingRead = pickle.load(f)
f.close()
eventTensor, sequenceTensor = trainingReadToTensor(trainingRead)
print(sequenceTensor.shape,eventTensor.shape)
eventTensor = eventTensor.reshape(1,eventTensor.shape[0],2)
sequenceTensor = sequenceTensor.reshape(1,sequenceTensor.shape[0],1)
print(sequenceTensor.shape,eventTensor.shape)
sequenceTensor = pad_sequences(sequenceTensor, dtype='float32', value=0,maxlen=eventTensor.shape[1],padding='post')
eventTensor = pad_sequences(eventTensor, dtype='float32', value=0,maxlen=eventTensor.shape[1],padding='post')
print(sequenceTensor.shape,eventTensor.shape)

print(model.predict([eventTensor,sequenceTensor]))
print(trainingRead.sixMers[0:100])
print(i)
