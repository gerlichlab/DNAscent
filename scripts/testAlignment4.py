...
logPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/trainingLog_alignment4.csv'
trainingReadLogPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/trainingReadsUsed_alignment4.txt'
valReadLogPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/valReadsUsed_alignment4.txt'
checkpointPath = '/home/mb915/rds/rds-mb915-notbackedup/data/2018_06_18_CAM_ONT_gDNA_BrdU_40_60_80_100_full/cnn_training/alignment4'
validationSplit = 0.2
...
#alignment actions
actionToInt = {'Match':0,'Insertion':1,'Deletion':2}
intToAction = {0:'Match',1:'Insertion',2:'Deletion'}

#alignment actions
actionToInt = {'CTCblank':0,'Dwell':1,'Insertion':2,'Deletion':3}
intToAction = {0:'CTCblank',1:'Dwell',2:'Insertion',3:'Deletion'}

#-------------------------------------------------
#
def trainingReadToTensor(t):

	#make the previous matches
	prevMatchesTensor = []
	for i, s in enumerate(t.sixMers[0:1000]):
		...
		for j in range(len(t.eventMean[i])):
			...
			oneHot[index] = 1
			prevMatchesTensor.append([t.eventMean[i][j], t.eventLength[i][j]] + oneHot)

	#just take the last 200 so we don't have to pad anything
	prevMatchesTensor = np.array(prevMatchesTensor[-1000:])

	#make the previous actions
	prevActionsTensor = []
	index = actionToInt['CTCblank']
	oneHot = [0]*4
	oneHot[index] = 1
	prevActionsTensor.append(oneHot)
	for i, s in enumerate(t.sixMers[1:1000]):
		...
		if s == 'NNNNNN':
			...
			if t.positions[i+1] != t.positions[i] + 1:

				for j in range(t.positions[i+1] - t.positions[i] - 1):
	...
	prevActionsTensor = np.array(prevActionsTensor[-1000:])			
	
	#make event tensor
	eventTensor = []
	for i, s in enumerate(t.sixMers[1000:]):
		for j in range(len(t.eventMean[i+1000])):	
			eventTensor.append([t.eventMean[i+1000][j], t.eventLength[i+1000][j]])
	...
	return prevMatchesTensor, prevActionsTensor, eventTensor, sequenceTensor
...
def trainingReadToLabel(t):
	...
	label =[]
	for i, s in enumerate(t.sixMers[200:2200]):
		...
		if s == 'NNNNNN':
			...
			if t.positions[i_fix] != t.positions[i_fix-1] + 1:
...
def buildModel(input_shape = (64, 64, 3), classes = 6):
	...
	input_actions = Input(name='actions_input',shape=(None,4))
	input_matches = Input(name='matches_input',shape=(None,6))

	#events stack
	E = Bidirectional(LSTM(8, return_sequences=True))(input_events)
	E = Bidirectional(LSTM(8, return_sequences=True))(E)
	E = Bidirectional(LSTM(8, return_sequences=True))(E)
	
	#base sequence stack
	S = Bidirectional(LSTM(8, return_sequences=True))(input_baseSequence)
	S = Bidirectional(LSTM(8, return_sequences=True))(S)
	S = Bidirectional(LSTM(8, return_sequences=True))(S)

	#past actions stack
	A = LSTM(16, return_sequences=True)(input_actions)
	A = LSTM(16, return_sequences=True)(A)
	A = LSTM(16, return_sequences=True)(A)

	#past matches stack
	M = LSTM(16, return_sequences=True)(input_matches)
	M = LSTM(16, return_sequences=True)(M)
	M = LSTM(16, return_sequences=True)(M)	

	D = Concatenate()([E,S,A,M])
	D = Dense(1000, activation="tanh")(D)
	D = Dropout(0.5)(D)
	D = TimeDistributed(Dense(4,activation='softmax'))(D)

	# Create model
	model = Model(inputs = [input_events,input_baseSequence,input_actions,input_matches], outputs = D, name='Alignment')
...
class DataGenerator(Sequence):
	...
	def __data_generation(self, list_IDs_temp):
		...
		for i, ID in enumerate(list_IDs_temp):
			...
			prevMatchesTensor, prevActionsTensor, eventTensor, sequenceTensor = trainingReadToTensor(trainingRead)
			label = trainingReadToLabel(trainingRead)

			M.append(prevMatchesTensor)
			A.append(prevActionsTensor)
			E.append(eventTensor)
			S.append(sequenceTensor)
			y.append(label)			

		M = np.array(M).reshape(self.batch_size,1000,6)
		A = np.array(A).reshape(self.batch_size,1000,4)
		...
		inputs = {'events_input': E, 'bases_input': S, 'actions_input': A,'matches_input': M}
