...
from tensorflow.keras.losses import CategoricalCrossentropy, MeanSquaredError, BinaryFocalCrossentropy, Loss, Reduction
...
logPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/trainingLog_DNN_39.csv'
trainingReadLogPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/trainingReads_DNN_39.txt'
valReadLogPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/validationReads_DNN_39.txt'
checkpointPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/checkpoints/39'
validationSplit = 0.1
...
class FuzzyLabel_CE_Loss(Loss):
	...
	def call(self, y_true, y_pred):
		...
		L = tf.TensorArray(tf.float32, size=batchSize)
		threshold = tf.constant([0.9])

		for i in range(batchSize):
			if tf.math.count_nonzero(y_true[i,:,2]) == 0 and tf.math.count_nonzero(y_true[i,:,3]) == 0: #thym only

				log_y_pred = tf.math.log(y_pred[i,:,:])
				elements = -tf.math.multiply_no_nan(x=log_y_pred, y=y_true[i,:,:])
				elements = tf.reduce_sum(elements,axis=-1)
				L = L.write(i, elements)
				#tf.print(elements, output_stream=sys.stderr)
			
			elif tf.reduce_any(tf.math.greater(y_true[i,:,2],threshold)): #brdu

				#apply transition matrix to label predictions
				T_labelNoise = tf.constant(([0.01, 1.-incorporationEstimate - 0.01, incorporationEstimate - 0.01, 0.01], 
				                            [0.01, 1.-incorporationEstimate - 0.01, incorporationEstimate - 0.01, 0.01],
				                            [0.01, 1.-incorporationEstimate - 0.01, incorporationEstimate - 0.01, 0.01],
				                            [0.01, 1.-incorporationEstimate - 0.01, incorporationEstimate - 0.01, 0.01]))
				T_pred = tf.linalg.matmul(y_pred[i,:,:], T_labelNoise, transpose_b=True)
				log_y_pred = tf.math.log(T_pred)
				elements_analogue = -tf.math.multiply_no_nan(x=log_y_pred, y=y_true[i,:,:])
				elements_analogue = tf.reduce_sum(elements_analogue,axis=-1)
				
				#apply normal categorical cross entropy to other positions
				log_y_pred = tf.math.log(y_pred[i,:,:])				
				elements_other = -tf.math.multiply_no_nan(x=log_y_pred, y=y_true[i,:,:])
				elements_other = tf.reduce_sum(elements_other,axis=1)
				#tf.print(elements_other, output_stream=sys.stdout)
				#tf.print(elements_other.shape)

				#find brdu and thym positions

				thymLoc = tf.math.greater(y_true[i,:,1], threshold)
				analogueLoc = tf.math.greater(y_true[i,:,2], threshold)
				bothLoc = tf.math.logical_or(thymLoc,analogueLoc)

				masked_seq_loss = tf.where(bothLoc, x=elements_analogue, y=elements_other)
				
				L = L.write(i, masked_seq_loss)
				#tf.print(masked_seq_loss, output_stream=sys.stderr)
			
			elif tf.reduce_any(tf.math.greater(y_true[i,:,3],threshold)): #edu

				#apply transition matrix to label predictions
				T_labelNoise = tf.constant(([0.01, 1.-incorporationEstimate - 0.01, 0.01, incorporationEstimate - 0.01],
				                            [0.01, 1.-incorporationEstimate - 0.01, 0.01, incorporationEstimate - 0.01],
				                            [0.01, 1.-incorporationEstimate - 0.01, 0.01, incorporationEstimate - 0.01],
				                            [0.01, 1.-incorporationEstimate - 0.01, 0.01, incorporationEstimate - 0.01]))
				T_pred = tf.linalg.matmul(y_pred[i,:,:], T_labelNoise, transpose_b=True)
				log_y_pred = tf.math.log(T_pred)
				...
				#find edu and thym positions
				thymLoc = tf.math.greater(y_true[i,:,1], threshold)
				analogueLoc = tf.math.greater(y_true[i,:,3], threshold)
				bothLoc = tf.math.logical_or(thymLoc,analogueLoc)
...
def trainingReadToLabel(t,whichSet):
	...
	if whichSet == 0.:
		for i, s in enumerate(t.analogueCalls):
			if s == '-': #not thymidine
				label.append([0.97, 0.01, 0.01, 0.01])
			else:
				label.append([0.01, 0.93, 0.03, 0.03])
		...
		for i, s in enumerate(t.analogueCalls):
			if s == '-': #not thymidine
				...
				label.append([0.01, 0.03, 0.93, 0.03])
		...
		for i, s in enumerate(t.analogueCalls):
			if s == '-': #not thymidine
				...
				label.append([0.01, 0.03, 0.03, 0.93])
...
class DataGenerator(Sequence):
	...
	def __data_generation(self, list_IDs_temp):
		...
		y = np.empty((self.batch_size, maxLen, 4), dtype=float)
...
model = buildModel((None,15,6), 4)
op = Adam(learning_rate=0.0001)
model.compile(optimizer=op, metrics=['accuracy'], loss=FuzzyLabel_CE_Loss())
print(model.summary())
