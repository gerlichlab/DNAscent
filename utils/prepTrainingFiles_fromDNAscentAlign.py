...
class trainingRead:
	...
	def __init__(self, read_sixMers, read_signals, read_modelMeans, read_positions, readID, label):
		...
		if len(read_kmers) > maxLen:
			...
			if switch:
				...
				for s in range(numReadSlices):
					readsLoaded += 1				
					tr = trainingRead(read_kmers[maxLen*s:maxLen*(s+1)], read_signals[maxLen*s:maxLen*(s+1)], read_modelSignal[maxLen*s:maxLen*(s+1)], read_positions[maxLen*s:maxLen*(s+1)], prevReadID, label)
