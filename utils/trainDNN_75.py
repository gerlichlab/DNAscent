...
logPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/trainingLog_DNN_75.csv'
trainingReadLogPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/trainingReads_DNN_75.txt'
valReadLogPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/validationReads_DNN_75.txt'
checkpointPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/checkpoints/75'
validationSplit = 0.1

f_checkpoint = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/checkpoints/43_pt2/weights.04-0.20.h5'

maxLen = 2000

#static params
incorporationEstimate = 0.8
truePositive_BrdU = incorporationEstimate*incorporationEstimate
truePositive_EdU = incorporationEstimate*incorporationEstimate
falsePositive = incorporationEstimate*(1.-incorporationEstimate)
...
def buildModel(classes, core_embedding_layer, residual_embedding_layer):
    ...
    #core_embedding_layer = Embedding(4**5+1, 32, trainable=False)
    #core_embedding_layer.set_weights(core_embedding_weights)
    ...
    #residual_embedding_layer = Embedding(4**4+1, 16, trainable=False)
    #residual_embedding_layer.set_weights(residual_embedding_weights)
    X_sequence_residual = residual_embedding_layer(residual_sequence_input)    
    
    #model means input
    model_input = Input(shape=(None,1))
    X_model = Conv1D(8, 5, strides = 1, padding='same', name = 'conv_model', kernel_initializer = glorot_uniform(seed=0))(model_input)
    
    signal_input = Input(shape=(None,maxEvents, 1))
    X_signal = TimeDistributed(Masking(mask_value=0.0))(signal_input)
    X_signal = TimeDistributed(Bidirectional(GRU(4,return_sequences=True)))(X_signal)
    X_signal = TimeDistributed(Bidirectional(GRU(4,return_sequences=False)))(X_signal)

    X = Concatenate(axis=-1)([X_sequence_core, X_sequence_residual, X_signal, X_model])
    ...
    # Create model
    model = Model(inputs = [core_sequence_input, residual_sequence_input, model_input, signal_input] , outputs = X, name='R10_BrdU_EdU')
	...
	if whichSet == 0.:
		...
		for i, s in enumerate(t.analogueCalls):
			if s == '-': #not thymidine
				...
				if score < incorporationEstimate:
...
class DataGenerator(Sequence):
	...
	def __data_generation(self, list_IDs_temp):
		...
		X3 = np.empty((self.batch_size, *self.dim[1]))
		X4 = np.empty((self.batch_size, *self.dim[2]))
		...
		for i, ID in enumerate(list_IDs_temp):
			...
			# input tensors
			core_sequence_tensor, residual_sequence_tensor, model_tensor, signal_tensor = trainingReadToTensor(trainingRead)
			X1[i,] = core_sequence_tensor
			X2[i,] = residual_sequence_tensor
			X3[i,] = model_tensor
			X4[i,] = signal_tensor.reshape(signal_tensor.shape[0], signal_tensor.shape[1], 1)
...
with strategy.scope():
	...
	#generator fit
	model.fit_generator(generator=training_generator, validation_data=validation_generator, epochs=50, verbose=1, callbacks=[es,chk,csv])
