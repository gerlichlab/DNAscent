...
logPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/trainingLog_DNN_35.csv'
trainingReadLogPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/trainingReads_DNN_35.txt'
valReadLogPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/validationReads_DNN_35.txt'
checkpointPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/checkpoints/35'
validationSplit = 0.1
...
class FuzzyLabel_CE_Loss(Loss):
	...
	def __init__(self, reduction=Reduction.NONE):
		super().__init__(reduction=Reduction.NONE,name='Fuzzy_CE_Loss')
	...
	def call(self, y_true, y_pred):
		...
		for i in range(batchSize):
			...
			elif tf.math.count_nonzero(y_true[i,:,3]) == 0: #brdu

				#apply transition matrix to label predictions
				T_labelNoise = tf.constant([0.0, 1.-incorporationEstimate, incorporationEstimate, 0.0],shape=[4,1])
				T_pred = tf.linalg.matmul(y_pred[i,:,:], T_labelNoise)
				T_pred = T_pred[:,0]
				
				ThymOrBrdU = tf.add(y_true[i,:,1], y_true[i,:,2]) #boolean vector of whether each position is thymidine or brdu (1) or otherwise (0)
				ElseOrEdU = tf.add(y_true[i,:,0], y_true[i,:,3]) #boolean vector of whether each position is A,C,G or EdU (1) or otherwise (0)		
				
				#mask predictions from transition matrix to thym or brdu positions
				T_pred_log_masked = tf.math.multiply_no_nan(-tf.math.log(T_pred), ThymOrBrdU)		
				
				#proceed with other positions as normal
				log_y_pred = tf.math.log(y_pred[i,:,:])				
				elements = -tf.math.multiply_no_nan(x=log_y_pred, y=y_true[i,:,:])
				elements = tf.reduce_sum(elements,axis=1)
				#tf.print(elements, output_stream=sys.stdout)
				#tf.print(elements.shape)
				elements_masked = tf.math.multiply_no_nan(elements, ElseOrEdU)				
			
				out = tf.add(T_pred_log_masked, elements_masked)
				L = L.write(i, out)
				#tf.print(out, output_stream=sys.stderr)
			
			elif tf.math.count_nonzero(y_true[i,:,2]) == 0 : #edu
			
				#apply transition matrix to label predictions
				T_labelNoise = tf.constant([0.0, 1.-incorporationEstimate, 0.0, incorporationEstimate],shape=[4,1])
				T_pred = tf.linalg.matmul(y_pred[i,:,:], T_labelNoise)
				T_pred = T_pred[:,0]
				
				ThymOrEdU = tf.add(y_true[i,:,1], y_true[i,:,3]) #boolean vector of whether each position is thymidine or edu (1) or otherwise (0)
				ElseOrBrdU = tf.add(y_true[i,:,0], y_true[i,:,2]) #boolean vector of whether each position is A,C,G or brdu (1) or otherwise (0)	
				
				#mask predictions from transition matrix to thym or brdu positions
				T_pred_log_masked = tf.math.multiply_no_nan(-tf.math.log(T_pred), ThymOrEdU)

				#proceed with other positions as normal
				log_y_pred = tf.math.log(y_pred[i,:,:])				
				elements = -tf.math.multiply_no_nan(x=log_y_pred, y=y_true[i,:,:])
				elements = tf.reduce_sum(elements,axis=1)
				elements_masked = tf.math.multiply_no_nan(elements, ElseOrBrdU)				

				out = tf.add(T_pred_log_masked, elements_masked)
				L = L.write(i, out)
				#tf.print(out, output_stream=sys.stderr)

		#tf.print(L.stack(), output_stream=sys.stderr)
		return L.stack()
...
model.compile(optimizer=op, metrics=['accuracy'], loss=FuzzyLabel_CE_Loss(reduction=Reduction.NONE))
print(model.summary())
