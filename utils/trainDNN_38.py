...
logPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/trainingLog_DNN_38.csv'
trainingReadLogPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/trainingReads_DNN_38.txt'
valReadLogPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/logs/validationReads_DNN_38.txt'
checkpointPath = '/home/mb915/rds/rds-boemo_3-tyMgmffheQw/workspace/R10_4_1_DNAtraining/checkpoints/38'
validationSplit = 0.1
	...
	def call(self, y_true, y_pred):
		...
		for i in range(batchSize):
			if tf.math.count_nonzero(y_true[i,:,2]) == 0 and tf.math.count_nonzero(y_true[i,:,3]) == 0: #thym only
				...
				T_labelNoise = tf.constant([0.01, 1.-incorporationEstimate - 0.01, incorporationEstimate - 0.01, 0.01],shape=[1,4])
				T_pred = tf.math.multiply_no_nan(x=y_pred[i,:,:], y=T_labelNoise)
				...
				#apply transition matrix to label predictions
				T_labelNoise = tf.constant([0.01, 1.-incorporationEstimate - 0.01, 0.01, incorporationEstimate - 0.01],shape=[1,4])
				T_pred = tf.math.multiply_no_nan(x=y_pred[i,:,:], y=T_labelNoise)
				log_y_pred = tf.math.log(T_pred)
				elements_analogue = -tf.math.multiply_no_nan(x=log_y_pred, y=y_true[i,:,:])
				elements_analogue = tf.reduce_sum(elements_analogue,axis=-1)
